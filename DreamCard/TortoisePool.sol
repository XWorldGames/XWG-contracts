...
contract TortoisePool is ERC721Holder, Ownable, Pausable{
    ...
    uint256 public totalHp;
    uint256 public settleBlock;
    uint16 public blockProduction;
    uint256 public totalReward;
    mapping (uint256 => uint256) public startBlockMapping;
    ...
    function setCurrentBlockProduction(uint16 number) external onlyOwner {
         blockProduction = number;
    }

    function setTeamRate(uint number) external onlyOwner {
        teamRate = number;
    }
    ...
    function settlePoolReward() public {
        uint256 currentBlock = block.number;

        if(currentBlock == settleBlock) {
            return;
        }

        totalReward = totalReward.add(currentBlock.sub(settleBlock).mul(blockProduction));

        settleBlock = currentBlock;
    }

    function getHp(uint256 tokenId) public view returns(uint256) {
        (uint32[] memory role, , ,) = nftRepository.get(tokenId);
        return role[5];
    }

    function queryReward(uint256 tokenId) public view returns (uint256 reward) {
        uint256 currentBlock = block.number;

        uint256 tokenStartBlock = startBlockMapping[tokenId];
        uint256 tokenMiningBlock = currentBlock.sub(tokenStartBlock);

        uint256 hp = getHp(tokenId);
        reward = totalReward.mul(hp).div(totalHp).mul(tokenMiningBlock).div(totalReward.div(blockProduction));
    }
    ...
    function distributeReward(uint256 reward) private {
        uint256 teamReward = reward.mul(teamRate).div(100);
        uint256 userReward = reward.sub(teamReward);

        rewardToken.mint(_msgSender(), userReward);
        rewardToken.mint(teamAddress, teamReward);
    }

    function withdrawalReward(uint256 tokenId) public whenNotPaused {
        require(_tokenOwners[tokenId] == _msgSender(), "NFTPool: transfer of token that is not own");
        ...
        startBlockMapping[tokenId] = block.number;
    ...
    function withdrawalReward() public whenNotPaused {
        ...
        distributeReward(userTotalReward);

        totalReward = totalReward.sub(userTotalReward);
    }
