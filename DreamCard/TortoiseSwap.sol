...
contract TortoiseSwap is Context, ERC721Holder, Governances, Pausable{
    ...
    function balanceOf(address owner) public view returns (uint256) {
        require(owner != address(0), "CrownStore: balance query for the zero address");
    ...
    function back(address target, uint256 tokenId) public onlyOwner {
        require(_tokenOwners[tokenId] == address(0), "NFTSwap:token is locked");

        nft.safeTransferFrom(address(this), target, tokenId);
    }

    function offline(uint256 tokenId) public whenNotPaused {
    ...
    function update(uint256 tokenId, address owner, uint32[] memory role, uint grade, uint[] memory skills, uint fiveElement) public onlyGovernance whenNotPaused {
        ...
        nftRepository.add(tokenId, role, grade, skills, fiveElement);
    }

    function online(uint256 tokenId, address owner, uint32[] memory role, uint grade, uint[] memory skills, uint fiveElements) public onlyGovernance whenNotPaused returns(uint256) {
        if(tokenId == 0) {
            return add(owner, role, grade, skills, fiveElements);
        } else {
            update(tokenId, owner, role, grade, skills, fiveElements);
            return tokenId;
        }
    }
